...
const getFriendlyMcpErrorMessage = (message: string): string => {
  if (LEGACY_MCP_ERROR_PATTERNS.some((pattern) => message.includes(pattern))) {
    return 'Legacy MCP SSE endpoints are not supported. Use a Streamable HTTP MCP endpoint instead.'
  }
  return message
}

export const getMcpErrorMessage = (error: unknown): string => {
